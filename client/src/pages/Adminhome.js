...
const Adminhome = () => {
  ...
  //to get all the books from database while loading the page
  useEffect(() => {
    (async () => {
      setLoader(true);
      try {
  ...
  const deleteBook = async (id, title) => {
    if (window.confirm(`Are you sure!, Do you want to delete ${title} book?`)) {
      setLoader(true);
      try {
        ...
        <div className="modalBackground">
          <div className="modalContainer">
            ...
            <div className="modalFooter">
              <button id="updateBtn">
                {/* while clicking edit button in modal it directs to editbook page along with book _id in paramenter */}
                <Link to={`/editbook/${modalData._id}`}>Edit</Link>
              </button>
              <button
                id="deleteBtn"
                onClick={() => deleteBook(modalData._id, modalData.title)}
              >
                Delete
              </button>
            </div>
